...
export default function ContactsField({
  ...
  const [selectedContact, setSelectedContact] = useState<HTMLLIElement | null>(
    null
  );
  const [isNoContacts, setNoContacts] = useState(false)

  useEffect(() => {
    contacts?.length ? setNoContacts(false) : setNoContacts(true)
  }, [contacts])
    ...
    <div className="w-fit mx-auto pt-3">
      <ul className="text-xl grid grid-cols-3 gap-5">
        {contacts ? (
          ...
          <li className="border rounded p-8 text-xl text-ventian-red">
            Ooops... I can't get data from the server!
          </li>
        )}
        {isModalDelete && (
          <ModalDelete
            clickDeleteNo={clickDeleteNo}
            clickDeleteYes={clickDeleteYes}
          />
        )}
        {isModalEdit && (
          <ModalEdit
            itemList={selectedContact}
            setModalEdit={setIsModalEdit}
            setContacts={setContacts}
            server={server}
          />
        )}
      </ul>
      {isNoContacts && (<div className="absolute top-1/4 text-xl ">No contacts. Add somebody...</div>)}
    </div>
  )
}
